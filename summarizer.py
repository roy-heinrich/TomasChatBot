...
async def summarize_and_store(input_filename: str = RAW_DOCS_FILE, output_filename: str = SUMMARY_FILE) -> str:
    """
    Streams docs from Supabase → OpenRouter (chunked summarization) →
    saves summary back to Supabase (summarized_text.md).
    """
    try:
        logger.info(f"⬇ Downloading {input_filename} from Supabase bucket {RAW_BUCKET}...")

        # Download raw docs
        raw_data = supabase.storage.from_(RAW_BUCKET).download(input_filename)
        if raw_data is None:
            logger.warning(f"⚠ {input_filename} not found in bucket {RAW_BUCKET}.")
            return ""
            ...
            for idx, chunk in enumerate(chunks):
                ...
                resp = await client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            {"role": "system", "content": "Summarize this document into clear Markdown with sections: Personnel, Departments, Events, Locations, Contact Info. Preserve names and titles."},
